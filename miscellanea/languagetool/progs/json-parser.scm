...
;; modified for use with TeXmacs by P. Joyez
;; changes indicated by comments ;;$
;; original file obtained from https://github.com/aconchillo/guile-json/blob/master/json/parser.scm
;; in may 2025 
...
;;$ masking
;;$(define-module (json parser)  -> texmacs-module below
...
(texmacs-module (json-parser))
;;
;; Miscellaneuos helpers
;;

(define (json-exception port)
  ;;$ (throw 'json-invalid port)) ; provoke error instead
  (display "json-invalid\n")
  halt) ; provoke error
...
(define (whitespace? c)
  (case c
    ((#\sp #\ht ;;$ #\lf guile 1.8 uses the notation #\newline 
        #\newline #\cr) #t)
    (else #f)))
...
(define (read-control-char port)
  (let ((ch (read-char port)))
    (cond
     ...
     ((eqv? ch #\/) #\/)
     ((eqv? ch #\b) #\bs)
     ;;$ ((eqv? ch #\f) #\ff)  guile 1.8 does not have #\\ff, using #xC instead
     ((eqv? ch #\f) #xC)
     ;;$ ((eqv? ch #\n) #\lf)  #\lf -> #\newline
     ((eqv? ch #\n) #\newline)
     ((eqv? ch #\r) #\cr)
...
(define-public (json-read port null ordered)
  (skip-whitespaces port)
  (let ((ch (peek-char port)))
    (cond
     ;; Unexpected EOF.
