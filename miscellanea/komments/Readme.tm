...
<\body>
  ...
  <strong|This package tries to fill this gap by hacking commenting tools
  that work hand in hand with the Versioning Tools>.

  My commenting tools were developed before the official Comment package
  ...
  This Komment package provides a new ``komment'' tag for highlighting text
  ...
  ``version-both'' tags (with the comment field possibly empty).
  ...
  <code*|my-init-texmacs.scm> (I made no menu, we mostly re-use the
  Versioning menu). Below I explain what these do, but they can be defined
  differently, of course.

  <subsubsection*|Introducing a komment : Ctl-m>

  Select some text beforehand and press Ctl-m. This brings up a text editing
  widget where you can enter the text of the komment. It can be as
  complicated as you want, contain equations... At the bottom of the widget,
  you can change the name of the komment author, and associate a color to
  that name.

  <image|./widget.png|0.6383w|||>

  <subsubsection*|Modifying a komment : Ctl-m>

  You can re-edit an existing komment by placing the cursor in it and
  pressing again Ctl-m. Be careful to close the widget (pressing OK) when you
  are done, or you could loose track of which widgets corresponds to which
  comment and your changes could get lost.

  <subsubsection*|Proposing a text change : Ctl-M (think ``Modify")>

  Select some text beforehand and press Ctl-M. This will automatically
  introduce a ``version-both tag'' inside a komment. The text that was
  ...
  deletion with no replacement text). Conversely, Ctl-M with no initial
  selection is a pure text addition.

  At that point the ``comment'' that could justify the change is empty; it
  can be added afterwards if needed, with Ctl-m.
  ...
  Only useful when a text contains both ``normal'' version tags and komments
  ...
  difference> or <code*|next difference> or the Ctl-up Ctl-down shortcuts).
  ...
  <kbd|Ctl-up> jump to previous komment

  <kbd|Ctl-down> jump to next komment

  <kbd|Ctl-left> display old text only

  <kbd|Ctl-right> display proposed new text only

  <kbd|Ctl-\|> display both old and new text

  <kbd|Ctl-1> retain old text and delete komment (accept proposed change)

  <kbd|Ctl-2> retain new text and delete komment (reject proposed change)

  If a komment contains no version tag, <kbd|Ctl-1> and <kbd|Ctl-2> both
  clear the komment.

  <subsection*|Installation>

  Copy the <verbatim|komments> folder to your \ <verbatim|~/.TeXmacs/plugins>
  user folder, then do <samp|Tool<math|\<rightarrow\>>Update<math|\<rightarrow\>>Plugins>.
  If you wan to change the keyboard shortcuts, you can edit them in the
  <verbatim|<verbatim|~/.TeXmacs/plugins>/komments/progs/init-komments.scm>
  file (if you want to find the exact location of your TeXmacs home folder in
  your system <hlink|use this file|https://raw.githubusercontent.com/texmacs/tm-forge/main/miscellanea/find-your-texmacs-paths.tm>).
...
<\auxiliary>
  <\collection>
    <\associate|toc>
      ...
      <with|par-left|<quote|2tab>|Introducing a komment : Ctl-m
      <datoms|<macro|x|<repeat|<arg|x>|<with|font-series|medium|<with|font-size|1|<space|0.2fn>.<space|0.2fn>>>>>|<htab|5mm>>
      <no-break><pageref|auto-4>>

      <with|par-left|<quote|2tab>|Modifying a komment : Ctl-m
      <datoms|<macro|x|<repeat|<arg|x>|<with|font-series|medium|<with|font-size|1|<space|0.2fn>.<space|0.2fn>>>>>|<htab|5mm>>
      <no-break><pageref|auto-5>>

      <with|par-left|<quote|2tab>|Proposing a text change : Ctl-M (think
      ``Modify") <datoms|<macro|x|<repeat|<arg|x>|<with|font-series|medium|<with|font-size|1|<space|0.2fn>.<space|0.2fn>>>>>|<htab|5mm>>
      <no-break><pageref|auto-6>>

      <with|par-left|<quote|2tab>|Circulating between different display
      styles of the comment field : Ctl-/
      <datoms|<macro|x|<repeat|<arg|x>|<with|font-series|medium|<with|font-size|1|<space|0.2fn>.<space|0.2fn>>>>>|<htab|5mm>>
      ...
      <with|par-left|<quote|2tab>|Navigating between koments, control their
